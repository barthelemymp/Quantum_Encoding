# -*- coding: utf-8 -*-
"""
Created on Wed Jul 24 15:36:43 2019
...
for b in range(1,4):
    ...
    CE_model = CE_id()
    model = AEQC_CRX(dev, 5, CE_model, 30, b)
    file_name = 'CRX_BC_AEQC_w'+str(5)+'_b'+str(b)+'_model_id'
    print(file_name)
    n_epoch = 30
    ...
    num.save('./CRX_results/'+file_name+'train_losses',train_losses)
    valid_losses = num.array(valid_losses)
    num.save('./CRX_results/'+file_name+'valid_losses',valid_losses)
    valid_acc = num.array(valid_acc)
    num.save('./CRX_results/'+file_name+'valid_acc',valid_acc)
    torch.save(model, './CRX_results/model'+file_name)
    print('saved')